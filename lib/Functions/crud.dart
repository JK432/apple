import 'package:applezero/Models/Apple.dart';
...
List<String> students =[];
  ...
  Stream<List<Apple>> readApple() => FirebaseFirestore.instance
      .collection('alerts')
      .snapshots()
      .map((snaphot) =>
      snaphot.docs.map((doc) => Apple.fromJson(doc.data())).toList());
  ...
  Delete( String id,BuildContext context)async{
     try{
      final FSinstance = FirebaseFirestore.instance
          .collection("alerts")
          .doc(id);
      await FSinstance.delete().then((value) => {
        ScaffoldMessenger.of(context)
            .showSnackBar(snackbar().snackBarSucess("Cleared"))
      });
    }on FirebaseException catch(e){
      String msg = e.message!;
      if (msg != null)
